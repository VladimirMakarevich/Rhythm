...
INSERT INTO dbo.Comments
	VALUES (1, N'Rocky Balboa', N'rocky@balboa.com', 1, GETDATE(), NULL, N'Очень простое объяснение: поля, содержащиеся в INCLUDE, хранятся только на листовом уровне, поля, содержащиеся в определении (до INCLUDE), являются частью ключа индекса, т.е. содержатся как на листовом, так и на уровне ветвей.')
INSERT INTO dbo.Comments
	VALUES (2, N'Admin', N'admin@gmail.com', 1, GETDATE(), NULL, N'First of all do I need to use the Inject attribute on all constructors that I want to use injection for. This seems like a really lame design? No you shouldn t have to do this at all actually. Since you work with ASP.NET MVC you can just install the Ninject.MVC3 Nuget package. This will get you started with a NinjectMVC3 class in the App_Start folder. You can use the RegisterServices method to register your interfaces/classes with Ninject. All controllers that have dependencies to those interfaces will then be automatically resolved by Ninject, there is no need for the Inject attribute. Do I need to create a Kernel then use that everywhere I pass in an injected class? No - what you are describing sounds more like the Service Locator pattern, not dependency injection - you will want to pass in your dependencies ideally in the constructor, instead of resolving them within particular classes using the kernel. There should be just one central composition root where the resolving is done, which is within the composition root in either the RegisterServices method mentioned above or a separate Ninject module instantiated there - the later approach will allow you a little more flexibility and modularity (no pun intended) in changing how you resolve your dependencies. Here s a good beginner s tutorial on dependency injection with Ninject and MVC3.')
INSERT INTO dbo.Comments
	VALUES (2, N'SuperAdmin', N'SuperAdmin@outlook.com', 0, GETDATE(), NULL, N'That doesnt matter because the concept are still the same. You still deal with the models.')
